...
pois.plot <- function(Genes, Reads, mse, id, HGMGS){
  ...
  plotly.data <- plotly.data + annotate("text", x=20, y=80, label = paste("MSE:", round(mse, 1)), parse=T, size=3)
  plotly.data <- plotly.data  + theme_minimal() + theme(legend.position="bottom")
  plotly.data  # for creating PDF (not interactive)
...
rank.mat <- function(id, gene.names, step, phase, threshold, t.start, t.end, n.replace = FALSE, mapped) {
 ...
 # plot(rowMeans(rank.matrix), xlim=c(0,(t.end-t.start)), ylim=c(min,max), xlab="SG index", ylab="SG rank mean", main=paste(id, threshold))
  ...
  #par(new=TRUE)
  #plot(rowMeans(rank.matrix), col="red",xlim=c(0,(t.end-t.start)), ylim=c(min,max), xlab="", ylab="")
